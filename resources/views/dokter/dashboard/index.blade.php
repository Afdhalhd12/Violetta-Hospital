...
@section('content')
    ...
    <div class="container my-4">
        ...
        <div class="dashboard-hero">
            <div>
                <h1 class="display-5 fw-bold mb-3">Doctor Dashboard</h1>
                <p class="lead">Manage patient appointments and update appointment status</p>
            </div>
        </div>
        ...
        <div class="dashboard-card">
            <div class="d-flex justify-content-between">
                <h3 class="section-title">Appointment list</h3>
                <a href="{{ route('dokter.dashboard.export') }}" class="btn btn-success mb-3"><i
                            ...
                            <tr>
                                ...
                                <td>
                                    <span
                                        ...
                                        @elseif($apt->status == 'confirmed') status-confirmed
                                        @elseif($apt->status == 'cancelled') status-cancelled
                                        @elseif($apt->status == 'completed') status-completed @endif">
                                        {{ ucfirst($apt->status) }}
                                    </span>
                                </td>
                                <td class="d-flex gap-2">
                                    <button type="button" class="btn btn-update action-btn"
                                        onclick="showUpdateModal({{ $apt }})">
                                        <i class="fas fa-edit me-1"></i> Update
                                    </button>

                                    <button type="button" class="btn btn-outline-secondary action-btn"
                                        onclick="showModal({{ $apt }})">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="updateForm" method="POST">
                ...
                @method('PATCH')
                <div class="modal-header">
                    <h5 class="modal-title text-light" id="modalUpdateLabel">Update Appointment Status</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="response" class="form-label">Response / Catatan</label>
                        <textarea name="response" id="response" rows="3" class="form-control" placeholder="Tulis respon dokter..."></textarea>
                    </div>
                </div>
...
@push('scripts')
    <script>
        function showModal(apt) {
            ...
            let content = `
                  ...
                  <li><strong>Patient Name:</strong> <span>${apt.patient.name}</span></li>
                  <li><strong>Date:</strong>
                  <span>${new Date(apt.date).toLocaleDateString('id-ID', {
                  day: '2-digit',
                  month: 'long',
                  year: 'numeric'
                  })}</span>
                 </li>
                 <li><strong>Time:</strong> <span>${apt.time}</span></li>
                 <li><strong>Status:</strong> <span class="${statusClass}">${apt.status}</span></li>
                 <li><strong>Notes:</strong>
            <div class="notes-content">${apt.notes ?? 'Tidak ada'}</div>
        </li>
    </ul>
    `;
    ...
    <script>
    function showUpdateModal(apt) {

        const form = document.getElementById('updateForm');
        form.action = `/dokter/dashboard/update/${apt.id}`;

        // isi data status & response di modal
        document.getElementById('status').value = apt.status ?? '';
        document.getElementById('response').value = apt.response ?? '';
